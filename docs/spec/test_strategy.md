...
test_scope:
  in_scope:
    ...
    - "章节滚动摘要、POV 边界与出场人物过滤组成的上下文一致性链路"
    - "小说创作质量自动校验（字数 3000-8000、悬念检测）与 3 轮自动扩写"
...
| 集成 / API 测试 |    ✅    | 涵盖 `/api/preferences`、向导分步链（`/api/novel/wizard` → `PATCH wizard` → `confirm-config` → `titles` → `confirm-title`）等端点契约 | Vitest (API Route Handlers 模拟测试) |
