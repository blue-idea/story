...
```typescript
import { pgTable, uuid, varchar, jsonb, timestamp, integer, boolean, text, primaryKey } from 'drizzle-orm/pg-core';
...
export const novelProfiles = pgTable('novel_profiles', {
  id: uuid('id').defaultRandom().primaryKey(),
  novelId: uuid('novel_id').references(() => novels.id, { onDelete: 'cascade' }).unique().notNull(),
...
export const chapters = pgTable('chapters', {
  id: uuid('id').defaultRandom().primaryKey(),
  novelId: uuid('novel_id').references(() => novels.id, { onDelete: 'cascade' }).notNull(),
