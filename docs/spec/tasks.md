...
- [x] **TASK-007 · 编写字数与悬念质量校验模块 (lib/writer/validator.ts)**
  - [x] 编写 validator.ts，检查生成的章节字数是否介于 3000-8000 字
...
- [x] **TASK-013 · 首页与快捷续写卡片 UI 开发**
  - [x] 开发首页，加载用户认证与偏好
  - [x] 美化界面，提供“继续上次创作”的渐显大卡片（包含标题、进度百分比、最后编辑日期）
  - [x] 提供“开启新小说”按钮，增加磨砂玻璃炫目微动效
...
- [x] **TASK-017 · 完稿阅读、内容修饰与打包导出页面开发**
  - [x] 小说完结后跳转至阅读界面，提供舒适流畅的双栏或单栏极简读书排版
  - [x] 允许在段落上直接双击或点击“修改”更新文本，提供“保存修改”按钮
  - [x] 支持选中文本后点击「润色选中内容」，调用 polish API 并预览替换（不自动覆盖全文）
  - [x] 顶部提供明显的“导出 Markdown”发光按钮，点击后触发整本打包下载
  ...
  **验收证据：** `/novel/[id]/read` 真实页面通过浏览器验收；阅读布局、编辑保存、选区润色预览与导出点击均通过，网络面板记录 `PUT /chapter/1`、`POST /chapter/1/polish`、`GET /export` 均返回 `200`。
  *需求: `requirements.md` 中 REQ-006
  验收标准：REQ-006-AC-001, REQ-006-AC-002, REQ-006-AC-003
  *测试类型: E2E
