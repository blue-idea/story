...
4. 创建章节文件，文件名格式：`第{XX}章-{章节标题}.md`（标题来自 `02-写作计划.json` 的 `title` 字段），基于大纲规划撰写正文（3000-8000字）
...
5. 执行逐章创作流程:
   ...
   b. 创建章节文件，文件名格式：`第{XX}章-{章节标题}.md`（标题来自 `02-写作计划.json` 的 `title` 字段），基于大纲规划撰写正文（3000-8000字）
